'''
Finite-temperature exact diagonalization (FTED) with grand canonical statistics.
...
class FTFCI:
    ...
    def kernel(self, h1e, h2e, norb, nelec, beta=np.inf, 
        ...
        if mu_gc is None:
            ...
            mu_gc = self.get_mu_gc_mf(h1e, h2e)
        ...
        for na in range(0, norb+1):
            if ab_symm:
                multiplier = 2.0
                nb_range = range(na+1, norb+1)
                parent_name = "sectors_unequal"
            else:
                nb_range = range(0, norb+1)
                parent_name = "sectors"
    ...
    def get_mu_gc_mf(self, h1e, h2e, beta=None, mu0=None):
        '''
        ...
        nelec_target = np.sum(self.nelec)
        if beta is None:
            beta = self.beta
        ...
        res = minimize(func, mu0, method="BFGS", jac=True, \
                    options={'disp':False, 'gtol':1e-3, 'maxiter':self.max_mu_cycle})
